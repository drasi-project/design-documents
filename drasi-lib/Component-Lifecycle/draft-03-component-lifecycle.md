...
* Draft 03 (classification mechanism; based on draft 02)
...
|---|---|---|
| **Managed runtime** | Graph node + stored runtime object. DrasiLib registers, provisions, starts, stops, updates, removes. Uses the full `ComponentStatus` machine. | `Source`, `Query`, `Reaction` |
| **Topology-only** | Graph node for visibility/edges. No runtime in the graph. Status stays `Added`. No start/stop/update path. | `BootstrapProvider`, `IdentityProvider` |
| **Infrastructure** | Not a graph node. Injected on the `DrasiLib` builder. Lifecycle is the instance's. | `IndexBackendPlugin`, `StateStoreProvider`, `WalProvider`, `SecretStoreProvider` |

Class is predicted by one fact: **is a runtime stored in the graph?** Yes → managed
runtime. Node but no runtime → topology-only. No node → infrastructure.
...
| **Cardinality** | `many-named` · `many-named-shared` · `singleton` · `attached` | How many exist per instance and how they are addressed. |
| **Runtime mutable** | `yes` · `no` | Can it be added/removed/replaced on a running `DrasiLib` instance? |
...
| Type | Class | Construction | Activity | Extension | Cardinality | Runtime mutable |
|---|---|---|---|---|---|---|
| `Source` | managed runtime | host | active | descriptor (`SourcePluginDescriptor`) | many-named | yes |
| `Query` | managed runtime | **lib** | active | **none** | many-named | yes |
| `Reaction` | managed runtime | host | active | descriptor (`ReactionPluginDescriptor`) | many-named | yes |
...
| Fact | True for |
|---|---|
| Graph node exists | managed runtime, topology-only |
| Runtime stored in graph (`set_runtime`) | managed runtime only |
| Full status transitions used | managed runtime only |
| `add` / `remove` / `update` on running instance | managed runtime only (`Runtime mutable = yes`) |
...
| Plugin id can appear in node metadata | host may set it for descriptor-backed managed runtimes; graph does not model plugins |
...
- **Cardinality / runtime mutable** answer: composition and clone/export shape.

"On the graph?" alone is insufficient — topology-only and managed runtime are both on the
graph but have different lifecycles. "Has a plugin descriptor?" alone is insufficient —
infrastructure and managed runtimes both can. The class × axes table is the working
classification.
