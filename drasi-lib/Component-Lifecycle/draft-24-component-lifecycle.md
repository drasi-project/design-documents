...
* Draft 24 (separate ownership and usage)
...
- their lifecycle
- whether they represented and/or contained in the ComponentGraph

| Component kind | Trait | Created By | Cardinality | Owner | Used By | Lifecycle |
|---|---|---|---|---|---|---|
| Bootstrap provider | `BootstrapProvider` | Host | 0 or 1 | Source | Source bootstrap logic | `Added` |
| Identity provider | `IdentityProvider` | Host | 0 or 1 | DrasiLib | Sources and Reactions | `Added` |
| Index backend | `IndexBackendPlugin` | DrasiLib default or Host | 0 or more; at most 1 default | DrasiLib | Queries | None |
| Query | `Query` | DrasiLib | 0 or more | DrasiLib | Reactions | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Reaction | `Reaction` | Host | 0 or more | DrasiLib | No other component | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Secret store | `SecretStoreProvider` | Host | 0 or 1 | DrasiLib | Configuration loading | None |
| Source | `Source` | Host | 0 or more | DrasiLib | Queries | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| State store | `StateStoreProvider` | Host or DrasiLib default | 1 | DrasiLib | Sources and Reactions | None |
| WAL provider | `WalProvider` | Host | 0 or 1 | DrasiLib | Sources | None |
