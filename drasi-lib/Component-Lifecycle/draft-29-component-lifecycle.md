...
* Draft 29 (identity provider cardinality note)
...
| Identity provider | `IdentityProvider` | Host | DrasiLib | 0..1 (See 2) | Sources and Reactions | Represented | `Added` |
...
2: The 0..1 cardinality applies to the Identity Provider owned by DrasiLib. Each Source and Reaction can also hold its own Identity Provider, which takes precedence over the DrasiLib provider. A single DrasiLib instance can therefore use multiple distinct Identity Providers, but the `identity-provider` graph node represents only the DrasiLib provider.
