...
* Draft 35 (index backend creation note)
...
Currently Component Kinds vary in terms of runtime characteristics such as:
- when they can be created and used
- what creates them
- what owns them once created
- the number of instances each Owner can hold (`cardinality`)
- their lifecycle
- whether they are represented by or contained in the ComponentGraph


| Kind | Creator | Owner | Created At | Cardinality | Used By | ComponentGraph | Lifecycle |
|---|---|---|---|---|---|---|---|
| Bootstrap provider | Host | Source | With Source | 0..1 | Source bootstrap logic | See 1 | `Added` |
| Identity provider | Host | DrasiLib, Source, or Reaction (See 2) | DrasiLib: Build; Source/Reaction: Build or Runtime | 0..1 | Sources and Reactions | See 2 | `Added` |
| Index backend | Host or DrasiLib (default) | DrasiLib | See 3 | 1.. | Queries | None | None |
| Query | DrasiLib | DrasiLib | Build or Runtime | 0.. | Reactions | Contained | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Reaction | Host | DrasiLib | Build or Runtime | 0.. | None | Contained | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Secret store | Host | DrasiLib | Build | 0..1 | Configuration loading | None | None |
| Source | Host | DrasiLib | Build or Runtime | 0.. | Queries | Contained | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| State store | Host or DrasiLib (default) | DrasiLib | Build | 1 | Sources and Reactions | None | None |
| WAL provider | Host | DrasiLib | Build | 0..1 | Sources | None | None |

1: The Bootstrap Provider instance is stored inside its Source. Its graph node is separate and metadata-only, and is created only through `with_bootstrap_for_source` or `register_bootstrap_provider`. Attaching an instance to a Source does not create the node, and the node does not reference the instance, so either can exist without the other.

2: An Identity Provider can be owned by DrasiLib, a Source, or a Reaction. DrasiLib can own one shared provider, which it supplies to Sources and Reactions that do not already own one; a provider owned by a Source or Reaction takes precedence. Only the DrasiLib-owned provider is represented in the ComponentGraph. The builder creates one metadata-only `identity-provider` node with `Authenticates` edges to the Sources and Reactions present during the build. The provider instance is not stored in the graph. Source- and Reaction-owned providers are not represented, and Sources and Reactions added after the build are not automatically connected to the node.

3: An `IndexBackendPlugin` is supplied during the DrasiLib build. DrasiLib does not create or share a default in-memory provider instance. When a Query has no configured backend, each time it starts it creates its own `InMemoryElementIndex`, `InMemoryResultIndex`, and `InMemoryFutureQueue`. These indexes are not shared between Queries and are recreated when a Query restarts. Its `InMemoryCheckpointStore` is retained separately across restarts.
