...
* Draft 25 (component graph presence)
...
`Represented` means the graph has a node but not the component instance. `Contained`
means the graph has both the node and the component instance. `Neither` means it has neither.

| Component kind | Trait | Created By | Cardinality | Owner | Used By | ComponentGraph | Lifecycle |
|---|---|---|---|---|---|---|---|
| Bootstrap provider | `BootstrapProvider` | Host | 0 or 1 | Source | Source bootstrap logic | Represented when registered | `Added` |
| Identity provider | `IdentityProvider` | Host | 0 or 1 | DrasiLib | Sources and Reactions | Represented | `Added` |
| Index backend | `IndexBackendPlugin` | DrasiLib default or Host | 0 or more; at most 1 default | DrasiLib | Queries | Neither | None |
