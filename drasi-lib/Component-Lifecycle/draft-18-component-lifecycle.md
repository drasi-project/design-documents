...
* Draft 18 (component cardinality)
...
| Component kind | Definition | Created By | Cardinality | Runtime representation | Lifecycle |
|---|---|---|---|---|---|
| Bootstrap provider | `BootstrapProvider` trait | Host | Zero or more; at most one per source | Passive instance owned by the source; optional metadata-only graph node; no instance in `runtimes` | No independent lifecycle; invoked as part of source/query bootstrap |
| Identity provider | `IdentityProvider` trait | Host (optional) | Zero or one per DrasiLib instance | Shared singleton passed to sources and reactions; metadata-only graph node when configured | No independent lifecycle; node remains `Added` |
| Index backend | `IndexBackendPlugin` trait | DrasiLib default or Host | Zero or more named providers; at most one default | Shared named provider plus index instances owned by individual queries; no graph node | No graph lifecycle; provider lives with DrasiLib and indexes are created during query provisioning |
| Query | `Query` trait | DrasiLib | Zero or more per DrasiLib instance | Named active instance stored in `runtimes` with a graph node and source dependencies | Full graph lifecycle: add, start, run, stop, reconfigure, error, remove |
| Reaction | `Reaction` trait | Host | Zero or more per DrasiLib instance | Named active instance stored in `runtimes` with a graph node and query dependencies | Full graph lifecycle: add, start, run, stop, reconfigure, error, remove |
| Secret store | `SecretStoreProvider` trait | Host (optional) | Zero or one per DrasiLib instance | Shared singleton used while resolving configuration; no graph node | No graph lifecycle; lifetime follows the DrasiLib instance |
| Source | `Source` trait | Host | Zero or more per DrasiLib instance | Named active instance stored in `runtimes` with a graph node | Full graph lifecycle: add, start, run, stop, reconfigure, error, remove |
| State store | `StateStoreProvider` trait | Host or DrasiLib default | One per DrasiLib instance | Shared singleton passed to sources and reactions; no graph node | No graph lifecycle; lifetime follows the DrasiLib instance |
| WAL provider | `WalProvider` trait | Host | Zero or one per DrasiLib instance | Shared singleton used by sources, with per-source registration; no graph node | No graph lifecycle; lifetime follows the DrasiLib instance |
