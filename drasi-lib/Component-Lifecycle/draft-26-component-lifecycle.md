...
* Draft 26 (bootstrap provider graph note)
...
| Kind | Trait | Creator | Cardinality | Owner | Used By | ComponentGraph | Lifecycle |
|---|---|---|---|---|---|---|---|
| Bootstrap provider | `BootstrapProvider` | Host | 0 or 1 | Source | Source bootstrap logic | See 1 | `Added` |
| Identity provider | `IdentityProvider` | Host | 0 or 1 | DrasiLib | Sources and Reactions | Represented | `Added` |
| Index backend | `IndexBackendPlugin` | Host or DrasiLib (default) | 0 or more; at most 1 default | DrasiLib | Queries | Neither | None |
| Query | `Query` | DrasiLib | 0 or more | DrasiLib | Reactions | Contained | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Reaction | `Reaction` | Host | 0 or more | DrasiLib | No other component | Contained | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Secret store | `SecretStoreProvider` | Host | 0 or 1 | DrasiLib | Configuration loading | Neither | None |
| Source | `Source` | Host | 0 or more | DrasiLib | Queries | Contained | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| State store | `StateStoreProvider` | Host or DrasiLib (default) | 1 | DrasiLib | Sources and Reactions | Neither | None |
| WAL provider | `WalProvider` | Host | 0 or 1 | DrasiLib | Sources | Neither | None |
