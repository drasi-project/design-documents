...
* Draft 23 (correct metadata-node states)
...
| Component kind | Trait | Created By | Cardinality | Ownership | Lifecycle |
|---|---|---|---|---|---|
| Bootstrap provider | `BootstrapProvider` | Host | 0 or 1 per Source | Owned by Source | `Added` |
| Identity provider | `IdentityProvider` | Host | 0 or 1 per DrasiLib | Singleton shared by Sources and Reactions | `Added` |
| Index backend | `IndexBackendPlugin` | DrasiLib default or Host | 0 or more per DrasiLib; 0 or 1 is the default | Owned by DrasiLib and shared by Queries | None |
| Query | `Query` | DrasiLib | 0 or more per DrasiLib | Owned by DrasiLib | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Reaction | `Reaction` | Host | 0 or more per DrasiLib | Owned by DrasiLib | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| Secret store | `SecretStoreProvider` | Host | 0 or 1 per DrasiLib | Singleton owned by DrasiLib | None |
| Source | `Source` | Host | 0 or more per DrasiLib | Owned by DrasiLib | `Added`, `Starting`, `Running`, `Stopping`, `Stopped`, `Reconfiguring`, `Error` |
| State store | `StateStoreProvider` | Host or DrasiLib default | 1 per DrasiLib | Singleton shared by Sources and Reactions | None |
| WAL provider | `WalProvider` | Host | 0 or 1 per DrasiLib | Singleton shared by Sources | None |
