...
* Draft 20 (concise creation ownership)
...
| Identity provider | `IdentityProvider` trait | Host | 0 or 1 per DrasiLib | Shared singleton passed to sources and reactions; metadata-only graph node when configured | No independent lifecycle; node remains `Added` |
...
| Secret store | `SecretStoreProvider` trait | Host | 0 or 1 per DrasiLib | Shared singleton used while resolving configuration; no graph node | No graph lifecycle; lifetime follows the DrasiLib instance |
