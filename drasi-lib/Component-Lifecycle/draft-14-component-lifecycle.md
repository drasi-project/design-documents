...
* Draft 14 (component-kind comparison)
...
These different Component Kinds are not consistent in terms of:
- where and how they are defined
- where and how they are created
- what they are at runtime
- their lifecycle

| Component kind | Definition | Creation | Runtime representation | Lifecycle |
|---|---|---|---|---|
| Bootstrap provider | `BootstrapProvider` trait; concrete types can be exposed through `BootstrapPluginDescriptor` | Created outside DrasiLib and attached to a source | Passive instance owned by the source; optional metadata-only graph node; no instance in `runtimes` | No independent lifecycle; invoked as part of source/query bootstrap |
| Identity provider | `IdentityProvider` trait; concrete types can be exposed through `IdentityProviderPluginDescriptor` | Created outside DrasiLib and supplied to the builder | Shared singleton passed to sources and reactions; metadata-only graph node when configured | No independent lifecycle; node remains `Added` |
| Index backend | `IndexBackendPlugin` trait; concrete types can be exposed through `IndexBackendPluginDescriptor` | Named providers are created outside DrasiLib and supplied to the builder; DrasiLib asks the selected provider to create per-query indexes | Shared named provider plus index instances owned by individual queries; no graph node | No graph lifecycle; provider lives with DrasiLib and indexes are created during query provisioning |
| Query | `Query` trait with internal `DrasiQuery` implementation; configured by `QueryConfig`; no plugin descriptor | Constructed inside DrasiLib from `QueryConfig` | Named active instance stored in `runtimes` with a graph node and source dependencies | Full graph lifecycle: add, start, run, stop, reconfigure, error, remove |
| Reaction | `Reaction` trait; concrete types can be exposed through `ReactionPluginDescriptor` | Created outside DrasiLib and passed through the builder or `add_reaction` | Named active instance stored in `runtimes` with a graph node and query dependencies | Full graph lifecycle: add, start, run, stop, reconfigure, error, remove |
| Secret store | `SecretStoreProvider` trait; concrete types can be exposed through `SecretStorePluginDescriptor` | Created outside DrasiLib and supplied to the builder | Shared singleton used while resolving configuration; no graph node | No graph lifecycle; lifetime follows the DrasiLib instance |
| Source | `Source` trait; concrete types can be exposed through `SourcePluginDescriptor` | Created outside DrasiLib and passed through the builder or `add_source` | Named active instance stored in `runtimes` with a graph node | Full graph lifecycle: add, start, run, stop, reconfigure, error, remove |
| State store | `StateStoreProvider` trait; no plugin descriptor | Supplied externally, or DrasiLib creates the default in-memory provider | Shared singleton passed to sources and reactions; no graph node | No graph lifecycle; lifetime follows the DrasiLib instance |
| WAL provider | `WalProvider` trait; no plugin descriptor | Created outside DrasiLib and supplied to the builder | Shared singleton used by sources, with per-source registration; no graph node | No graph lifecycle; lifetime follows the DrasiLib instance |
