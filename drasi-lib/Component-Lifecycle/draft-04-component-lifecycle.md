...
* Draft 04 (clarify "runtime" terminology; based on draft 03)
...
Types differ along a small set of independent axes. The **class** is the primary bucket;
the other axes are attributes.

**Terminology — "component object".** In this document, *component object* means the live
trait object for one component — typically `Arc<dyn Source>`, `Arc<dyn Query>`, or
`Arc<dyn Reaction>`. The graph keeps those in a side map field literally named `runtimes`,
accessed by `set_runtime` / `get_runtime` / `take_runtime`. That is a code name for "stored
live objects." It does **not** mean the OS process, the Tokio runtime, or "the DrasiLib
execution environment." Where the code API name matters, this doc says `runtimes` map or
`set_runtime`.

### Class (primary)

What relationship does the type have to DrasiLib's control plane?

| Class | Definition | Members |
|---|---|---|
| **Lifecycle-managed** | Graph node **and** a component object in the graph's `runtimes` map. DrasiLib registers, provisions, starts, stops, updates, removes. Uses the full `ComponentStatus` machine. | `Source`, `Query`, `Reaction` |
| **Topology-only** | Graph node for visibility/edges. No entry in `runtimes`. Status stays `Added`. No start/stop/update path. | `BootstrapProvider`, `IdentityProvider` |
| **Infrastructure** | Not a graph node. Injected on the `DrasiLib` builder. Lifecycle is the DrasiLib instance's. | `IndexBackendPlugin`, `StateStoreProvider`, `WalProvider`, `SecretStoreProvider` |

Class is predicted by one fact: **does the graph hold a component object for this id in
`runtimes`?** Yes → lifecycle-managed. Node but no `runtimes` entry → topology-only. No
node → infrastructure.
...
| Axis | Values | Meaning |
|---|---|---|
| **Construction** | `host` · `lib` | Who creates the object. `host` = caller builds it and hands it in. `lib` = DrasiLib builds it from config it holds. |
| **Activity** | `active` · `passive` | `active` = DrasiLib drives `start`/`stop`; the object does ongoing work and reports status. `passive` = used on demand by something else; DrasiLib does not run an independent lifecycle for it. |
| **Extension** | `descriptor` · `static-only` · `none` | How implementations are supplied. `descriptor` = a `*PluginDescriptor` exists (usable statically or via `drasi-host-sdk` dynamic load). `static-only` = trait only, no descriptor in the plugin SDK. `none` = not an extensible plugin kind. |
| **Cardinality** | `many-named` · `many-named-shared` · `singleton` · `attached` | How many exist per DrasiLib instance and how they are addressed. |
| **Live-mutable** | `yes` · `no` | Can it be added/removed/replaced on an already-built `DrasiLib` instance? |

**Cardinality values:**

| Value | Meaning |
|---|---|
| `many-named` | Independent instances, each with an id; independently removable |
| `many-named-shared` | Named shared providers; selected by name; not per-consumer instances on the graph |
| `singleton` | At most one per `DrasiLib` instance |
| `attached` | Bound to another component (bootstrap → source); not independently addressed by lib ops |

**Activity note:** bootstrap providers do work during source/query bootstrap, but that work is
invoked by the source/query path. They are `passive` under this definition because DrasiLib
does not start/stop them as independent components.

### Applied to all nine types

| Type | Class | Construction | Activity | Extension | Cardinality | Live-mutable |
|---|---|---|---|---|---|---|
...
| Component object in `runtimes` map (`set_runtime`) | lifecycle-managed only |
...
| Config reproducible without live object | `Query` only (`Construction = lib`) |
| Plugin id can appear in node metadata | host may set it for descriptor-backed lifecycle-managed types; graph does not model plugins |
...
- **Class** answers: what does DrasiLib actually manage?
- **Construction** answers: whose failure is a construct-time failure, and who holds config?
- **Activity** answers: does "started successfully" mean anything for this type?
- **Extension** answers: can a new implementation arrive via plugin load, or only by linking?
- **Cardinality / live-mutable** answer: composition and clone/export shape.

"On the graph?" alone is insufficient — topology-only and lifecycle-managed are both on the
graph but have different lifecycles. "Has a plugin descriptor?" alone is insufficient —
infrastructure and lifecycle-managed both can. The class × axes table is the working
classification.
...
4. the live component object for types that store one (`runtimes` map)
...
| `runtimes` | `component_id → Box<dyn Any + Send + Sync>` (live component objects) |
...
### Component object store (`runtimes` map)
...
The graph does not construct component objects. It only stores what callers put there.
`GraphTransaction` does not cover the `runtimes` map.

Only `Source`, `Query` and `Reaction` ever get a component object stored (six `set_runtime`
call sites in the managers). `BootstrapProvider` and `IdentityProvider` get a node and
nothing else.

**Registry-first:** the node must exist before a component object is stored against it.
...
Registration does **not** store a component object. Callers that need one call `set_runtime`
later.
...
transaction. If storing/initializing the component object fails after commit, callers must
compensate with `deregister` / `remove_component`.
...
| Component objects (`runtimes`) | `set_runtime`, `get_runtime`, `take_runtime`, `has_runtime` |
...
lifecycle: no component object is stored for them by current callers, and nothing
...
- **`Query` is the only `Construction = lib` type.** Built from `QueryConfig`. Per-query
  index *instances* are also created inside DrasiLib, via a host-supplied
  `IndexBackendPlugin` factory (`create_indexes(query_id)`).
- **`IdentityProvider` is topology-only *and* infrastructure-shaped.** Injected on the
  builder as a singleton, and also registered as a graph node under hard-coded id
  `identity-provider`. `Authenticates` edges are drawn once at build against components
  present then; components added later are never linked, so those edges go stale. The
  node is unused by lifecycle code ("reserved for future use").
- **`BootstrapProvider` is `attached`.** Host sets it on the source before hand-over. The
  optional graph node is for topology only. Drasi Server treats bootstraps as top-level
  named config components — a lib/server mismatch (see Bootstrap provider section).
...
| Provision | DrasiLib | `initialize()` supplies the source context. No external I/O. Object stored via `set_runtime`. |
...
| Update | DrasiLib | Component object replaced via `Reconfiguring`. |
...
It has no component object in `runtimes`, no `initialize()`/`start()`/`stop()`, no add,
...
| Provision | DrasiLib | `initialize()` supplies the query context. Object stored via `set_runtime`. |
...
| Provision | DrasiLib | `initialize()` supplies the reaction context. No external I/O. Object stored via `set_runtime`. |
...
1. **Only three of the nine component types are lifecycle-managed.** Bootstrap and identity
   providers have graph presence without graph behavior; the four injected providers have
   neither. The predictor is whether a component object is stored in the graph's `runtimes`
   map.
...
14. **`GraphTransaction` does not cover the `runtimes` map.** Failure while storing or
    initializing a component object after commit requires compensating `deregister` /
    `remove_component`.
